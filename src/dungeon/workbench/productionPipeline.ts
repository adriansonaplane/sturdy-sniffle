...
export async function generateProductionCatacombsDungeon(config: DungeonConfig = DEFAULT_DUNGEON_CONFIG): Promise<ResolvedDungeon> {
  ...
  const resolved: ResolvedDungeon = {
    ...
    assetUsage: [...new Map(construction.assetResolutions.map(a => [a.resolvedAssetId ?? (a.requested.family ?? a.requested.assetId ?? 'unknown'), a])).entries()].map(([assetId, a]) => ({ assetId, version: '1', placementIds: construction.records.filter(r => r.resolvedAssetId === assetId || r.requestedAssetFamily === assetId).map(r => r.id), status: a.state === 'AUTHORED' ? 'authored' : a.state === 'FALLBACK' ? 'fallback' : 'missing_blocked' })),
